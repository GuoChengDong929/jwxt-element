<template>
    <div>
        <div style="margin: 20px 5px 10px 5px;padding: 5px;">
            ...
            <el-input type="text" placeholder="请输入上机题题干关键字" v-model="params.upperContent" style="width:217px;margin-right:10px"></el-input>


            <el-button plain type="success" @click="getList(params)"> <span class="el-icon-search"></span>查询</el-button>
            <el-button plain type="info" @click="params = {page:1,size:10,total:0,type:'4'},getList(params)"> <span class="el-icon-refresh-left"></span>清空</el-button>
			
		</div>
        <div style="margin: 5px;padding: 5px;">
            <el-table :data="upperData" :loading="loading" element-loading-text="玩了命的加载...." element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)">
                <el-table-column type="index" align="center" width="80px">
                    <template slot="header">
                        <el-button size="mini" plain type="success" @click="openWindow('upperChild',null)"><span class="el-icon-plus"></span></el-button>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="upperContent" label="题干" align="center" show-overflow-tooltip></el-table-column>
            </el-table>
        </div>
        ...
        <upper-child ref="upperChild"></upper-child>
    </div>
</template>
<script>
    ...
    export default {
        ...
        components:{
            UpperChild:()=>import("@/views/academic/question/upper-child.vue")
        },
        data(){
            return{
                loading:true,
                ...
                chapterData:[],
                companyData:[],
                params:{page:1,size:10,total:0,type:'4'}
            }
        },
        methods:{
            openWindow(name,data){
                this.$refs[name].upper = {};
            ...
            getList(data){
                getList(data).then(res=>{
                    if(res.data.success){
                        this.upperData = res.data.data.rows;
