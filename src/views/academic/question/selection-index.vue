<template>
    <div>
        <div style="margin: 20px 5px 10px 5px;padding: 5px;">
            ...
            <el-input type="text" placeholder="请输入多选题题干关键字" v-model="params.selectionContent" style="width:217px;margin-right:10px"></el-input>

            <el-button plain type="success" @click="getList(params)"> <span class="el-icon-search"></span>查询</el-button>
            <el-button plain type="info" @click="params = {page:1,size:10,total:0,type:'2'},getList(params)"> <span class="el-icon-refresh-left"></span>清空</el-button>
			
		</div>
        <div style="margin: 5px;padding: 5px;">
            <el-table :data="selectionData" :loading="loading" element-loading-text="玩了命的加载...." element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)">
                <el-table-column type="index" align="center" width="80px">
                    <template slot="header">
                        <el-button size="mini" plain type="success" @click="openWindow('selectionChild',null)"><span class="el-icon-plus"></span></el-button>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="selectionContent" label="题干" align="center" show-overflow-tooltip></el-table-column>
                <el-table-column label="操作" align="center">
                    <template slot-scope="scope">
                        <el-button type="success" plain size="mini" @click="openWindow('selectionChild',scope.row)"><span class="el-icon-edit"></span>修改</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        ...
        <selection-child ref="selectionChild"></selection-child>
    </div>
</template>
<script>
    ...
    export default {
        ...
        components:{
            SelectionChild:()=>import("@/views/academic/question/selection-child.vue")
        },
        data(){
            return{
                loading:true,
                ...
                chapterData:[],
                companyData:[],
                params:{page:1,size:10,total:0,type:'2'}
            }
        },
        methods:{
            openWindow(name,data){
                if(data == null){
                    this.$refs[name].selection = {};
                    this.$refs[name].selectionAskList = [];
                }else{
                    this.$refs[name].selection = data;
                }
            ...
            getList(data){
                getList(data).then(res=>{
                    if(res.data.success){
                        this.selectionData = res.data.data.rows;
