<template>
    <div>
        <div style="margin: 20px 5px 10px 5px;padding: 5px">
            ...
            ></el-date-picker>
            <el-button type="success" plain @click="search"> <i class="el-icon-search"></i>查询 </el-button>
            <el-button type="danger" plain @click="comparison"> <i class="el-icon-paperclip" style="margin-right: 5px"></i>考核对比</el-button>
        </div>
...
<script>
    ...
    import { getMonthScoresByDate,comparisonMonth } from "@/api/academic/supervise";

    import { formatDateNoWeekNoSeconds } from "@/utils/tools";

    export default {
        name:"week-supervise-index",
        data(){
            return {
                loading:false,
        ...
        methods:{
            comparison(){
                ...
                comparisonMonth(this.params).then(res=>{
                    if(res.data.success){
            ...
            search(){
                ...
                getMonthScoresByDate(this.params).then(res=>{
                    if(res.data.success){
            ...
            init() { //初始化图表
                ...
                myChart.setOption({
                    title: { //标题项
                        show: true, //是否显示标题
                        text: '月考对比(相比前一个月,低1个百分点将扣除1分考核)',
